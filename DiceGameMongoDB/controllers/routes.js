...
const router = app => {
    ...
    app.post('/players', async (request, response) => {
        ...
        console.log(result);
        response.status(201).send(`Jugador afegit:\n ${JSON.stringify(result)}`);
        });
    ...
    app.post('/players/:id/games', async (request, response) => {
        ...
        console.log(result);
        response.status(201).send(`Tirada registrada:\n ${JSON.stringify(result)}`);
    });

    // Modifica nom de jugador
    app.put('/players/:id', async (request, response) => {
        const result = await services.updatePlayerById(request.params.id, request.body.playername);
        console.log(result);
